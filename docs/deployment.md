...
- **Kiosk full screen**: the app ships a web app manifest declaring `scope: "/"`,
  which is what keeps a home-screen launch full screen on every page instead of
  only on `/`. iOS caches that scope **when the home-screen icon is created**, so
  after a deploy that changed the manifest the icon must be removed and added
  again — an existing icon keeps its old behaviour no matter what the server
  sends.
